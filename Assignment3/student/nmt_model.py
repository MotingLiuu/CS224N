...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        h_enc = torch.cat((hidden_forward, hidden_backward), dim=1)
        cell_forward = c_n[0, :, :]
        cell_backward= c_n[1, :, :]
        c_enc = torch.cat((cell_forward, cell_backward), dim=1)
        dec_init_state = (self.h_projection(h_enc), self.c_projection(c_enc))
        ### END YOUR CODE
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for time in Y:
            ...
            dec_state, o_t, e_t = self.step(Ybar_t, dec_state=dec_state, enc_hiddens=enc_hiddens, enc_hiddens_proj=enc_hiddens_proj, enc_masks=enc_masks)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ###         https://pytorch.org/docs/stable/generated/torch.squeeze.html
        ### YOUR CODE HERE (~3 Lines)
        ### Step 1: Apply the decoder
        dec_state = self.decoder(Ybar_t, dec_state)
        dec_hidden, dec_cell = dec_state
        e_t = torch.bmm(dec_hidden.unsqueeze(dim=1), enc_hiddens_proj.transpose(1, 2)).squeeze(dim=1)
        ### END YOUR CODE
